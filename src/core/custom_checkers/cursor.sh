...
cursor::parse_config() {
    ...
    local cache_key="cursor_$(hashes::generate "$app_config_json")"
...
cursor::resolve_download() {
    ...
    local final_name_part="$(basename "${final_url%%[\?#]*}")"
    local chosen_name="${filename:-$final_name_part}"
    local artifact_type="$(cursor::detect_artifact_type "$chosen_name")"
...
cursor::resolve_from_html() {
    ...
    if best_url="$(cursor::select_best_url "${candidates[@]}")"; then
        ...
        local final_name_part="$(basename "${resolved_url%%[\?#]*}")"
        local chosen_name="${filename:-$final_name_part}"
        local artifact_type="$(cursor::detect_artifact_type "$chosen_name")"
