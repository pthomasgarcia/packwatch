    ...
    if [[ "$status" == "success" ]] && updates::is_needed "$installed_version" "$latest_version"; then
        ...
        case "$install_type" in
            ...
            "appimage")
                ...
                local expected_checksum_from_output checksum_algorithm_from_output
                expected_checksum_from_output=$(echo "$custom_checker_output" | jq -r '.expected_checksum // empty')
                checksum_algorithm_from_output=$(echo "$custom_checker_output" | jq -r '.checksum_algorithm // "sha256"')
                ...
                updates::process_installation \
                    ...
                    "${expected_checksum_from_output:-${app_config_ref[checksum_url]:-}}" \
                    "${checksum_algorithm_from_output:-${app_config_ref[checksum_algorithm]:-sha256}}" \
