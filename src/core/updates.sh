...
updates::on_download_start() {
    ...
    loggers::print_ui_line "  " "→ " "Downloading $(_bold "$app_name")..." >&2  # Redirect to stderr
...
updates::on_download_progress() {
    ...
    loggers::print_ui_line "  " "⤓ " "Downloading $(_bold "$app_name"): $percent% ($(_format_bytes "$downloaded") / $(_format_bytes "$total"))" >&2  # Redirect to stderr
...
updates::on_download_complete() {
    local app_name="$1"
    local file_path="$2"
    loggers::print_ui_line "  " "✓ " "Download for $(_bold "$app_name") complete." _color_green >&2  # Redirect to stderr
    loggers::log_message "INFO" "Download complete for $app_name: $file_path"
}

updates::on_install_start() {
    local app_name="$1"
    loggers::print_ui_line "  " "→ " "Preparing to install $(_bold "$app_name")..." >&2  # Redirect to stderr
    loggers::log_message "INFO" "Starting installation for $app_name."
}

updates::on_install_complete() {
    local app_name="$1"
    loggers::print_ui_line "  " "✓ " "$(_bold "$app_name") installed/updated successfully." _color_green >&2  # Redirect to stderr
...
updates::on_install_skipped() {
    local app_name="$1"
    loggers::print_ui_line "  " "🞨 " "Installation for $(_bold "$app_name") skipped." _color_yellow >&2  # Redirect to stderr
...
updates::_prepare_deb_file() {
    ...
    if [[ -n "$deb_file_to_install" && -f "$deb_file_to_install" ]]; then
        ...
        local create_output
        create_output=$(systems::create_temp_file "${base_filename}" 2>&1)
        if [[ $? -ne 0 ]]; then return 1; fi
...
updates::process_deb_package() {
    ...
    # Prepare DEB file
    local final_deb_path
    final_deb_path=$(updates::_prepare_deb_file "$app_name" "$download_url" "$deb_file_to_install" "$expected_checksum" "$checksum_algorithm")
    if [[ $? -ne 0 ]]; then return 1; fi
...
updates::check_github_deb() {
    ...
    local api_response
    api_response=$("$UPDATES_GET_LATEST_RELEASE_INFO_IMPL" "$repo_owner" "$repo_name") # DI applied
    if [[ $? -ne 0 ]]; then
    ...
    latest_release_json=$("$UPDATES_GET_JSON_VALUE_IMPL" "$api_response" '.[0]' "$name") # DI applied
    if [[ $? -ne 0 ]]; then
    ...
    latest_version=$(repositories::parse_version_from_release "$latest_release_json" "$name")
    if [[ $? -ne 0 ]]; then
    ...
    if updates::is_needed "$installed_version" "$latest_version"; then
        ...
        expected_checksum=$(repositories::find_asset_checksum "$latest_release_json" "$download_filename")
        if [[ $? -ne 0 ]]; then
    ...
    if [[ -n "$github_repo_owner" ]] && [[ -n "$github_repo_name" ]]; then
        ...
        if api_response=$("$UPDATES_GET_LATEST_RELEASE_INFO_IMPL" "$github_repo_owner" "$github_repo_name"); then # DI applied
            ...
            if [[ $? -eq 0 && -n "$latest_release_json" ]]; then
                ...
                expected_checksum=$(repositories::find_asset_checksum "$latest_release_json" "$download_filename_from_url")
...
updates::check_flatpak() {
    ...
    loggers::print_ui_line "  " "→ " "Checking Flatpak for $(_bold "$name")..."
...
updates::check_application() {
    ...
    if ! updates::_validate_app_config "$app_type" "_current_app_config"; then
        loggers::print_ui_line "  " "✗ " "Configuration error: Missing required fields." _color_red
        counters::inc_failed
