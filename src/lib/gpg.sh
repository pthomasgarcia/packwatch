...
_gpg_prepare_temp_keyring_with_key() {
    ...
    local tmp; tmp=$(mktemp -d) || return 1; chmod 700 "$tmp"
    case "$source" in
      url)
        local f="$tmp/key.asc"
        curl -fsSL "$key_url" -o "$f" || { rm -rf "$tmp"; return 1; }
        gpg --homedir "$tmp" --import "$f" >/dev/null 2>&1 || { rm -rf "$tmp"; return 1; }
        ;;
      keyserver)
        gpg --homedir "$tmp" --keyserver hkps://keyserver.ubuntu.com --recv-keys "$key_id" >/dev/null 2>&1 || { rm -rf "$tmp"; return 1; }
        ;;
      wkd)
        GNUPGHOME="$tmp" gpg --auto-key-locate clear,wkd --locate-keys "$key_id" >/dev/null 2>&1 || { rm -rf "$tmp"; return 1; }
        ;;
      user_keyring)
        echo "${ORIGINAL_HOME:-$HOME}/.gnupg"; return 0;;
      *) rm -rf "$tmp"; return 1;;
    esac
    echo "$tmp"
}
...
gpg::verify_detached() {
    ...
    local home; home=$(_gpg_prepare_temp_keyring_with_key "$source" "$key_id" "$key_url") || return 1
    local actual_fp; actual_fp=$(GNUPGHOME="$home" gpg --fingerprint --with-colons "$key_id" 2>/dev/null | awk -F: '/^fpr:/ {print $10}' | head -n1)
    local nx="${expected_fp//[[:space:]]/}" na="${actual_fp//[[:space:]]/}"
    if [[ -z "$na" || "$na" != "$nx" ]]; then
        [[ "$home" != "${ORIGINAL_HOME:-$HOME}/.gnupg" ]] && rm -rf "$home"
        errors::handle_error "GPG_ERROR" "Fingerprint mismatch. Expected $expected_fp, got ${actual_fp:-<none>}"; return 1
    fi
